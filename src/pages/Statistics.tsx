...
import { Card, Flex, Select, SelectProps, Tabs, TabsProps } from 'antd';
...
export const Statistics = () => {
  ...
  const { data: projectsemployee } = useGetProjectByEmployeeQuery({ id: currentuser?.data?.id || '', year: date });
  ...
  useEffect(() => {
    ...
    const datas= response?.data?.filter((x) => parseInt(x.date.toString().split('-')[0]) == date) || [];
    setTotalTimeEntry(datas.reduce((acc, current) => acc + current.hours, 0).toString() || '0',);
    datas.forEach((x) => {
    ...
    setLeaveday(projectsemployee?.data.reduce((acc, current) => acc + current.limit, 0).toString() || '0',)
  }, [date]);
