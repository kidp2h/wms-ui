...
export const CardManager = () => {
 ...
 const{data :responseTimeEntry} = useGetTimeEntrysQuery()
    ...
    const [LeaveDayused, setLeaveDayused] = useState<string>('0');
      useEffect(() => {
        if (response?.data) {
          ...
          const projects: any = response?.data.filter(x=>x.type== TypeProject.PROJECT) || [];
            setTotalProject(projects.length.toString());
            const totalTimeEntry: any = responseTimeEntry?.data?.reduce((acc: any, time: any) => {
                if( time.project.type == TypeProject.PROJECT)
                acc += time.hours;
                return acc;
              }, 0);
              console.log(totalTimeEntry)
              setTotalTimeEntry(totalTimeEntry.toString());
        } 
        const employees: any = responseTimeEntry?.data?.reduce((acc: any, employee: any) => {
            if (!acc.includes(employee.employeeId) &&new Date(employee.date.toString()).getFullYear() === new Date().getFullYear() ) {
                acc.push(employee.employeeId);
              }
              return acc;
        },[]);
        const leaves: any = responseTimeEntry?.data?.filter(x=>x.project.type== TypeProject.LEAVE &&new Date(x.date.toString()).getFullYear() === new Date().getFullYear()) || [];

        const totalLeaveDay: any = leaves.reduce((acc: any, time: any) => {
            if(new Date(time.date.toString()).getFullYear() === new Date().getFullYear())
            acc += time.hours;
            return acc;
          }, 0);
          setLeaveDayused(( parseFloat((totalLeaveDay / 8).toFixed(1)) ).toString());
          
        setEmployees(employees.length.toString());
      }, [response]);
